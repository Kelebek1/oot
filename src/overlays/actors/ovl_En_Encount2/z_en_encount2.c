...
void func_80A07A4C(EnEncount2* this, GlobalContext* globalCtx) {
    ...
    if (this->unk_15A == 0) {
        if ((1500.0f < player->actor.posRot.pos.y) && (-700.0f < player->actor.posRot.pos.x) &&
            (player->actor.posRot.pos.x < 100.0f) && (player->actor.posRot.pos.z < -1290.0f) &&
            (-3600.0f < player->actor.posRot.pos.z)) {
            tmp1 = 1;
        }
    } else {
        if ((this->actor.xzDistFromLink < 700.0f) && (Flags_GetSwitch(globalCtx, 0x37) != 0) &&
            ((globalCtx->sceneNum == 0x4F) || (globalCtx->sceneNum == 0x1A) || (globalCtx->sceneNum == 0xE) ||
             (globalCtx->sceneNum == 0xF)) &&
            (this->unk_15C == 0)) {
            tmp1 = 2;
        }
    }
    ...
    switch (tmp1) {
        ...
        case 1:
            if ((this->unk_154 == 1) || ((this->unk_180 == 0) && (this->unk_184 == 0))) {
...
void func_80A07CA4(EnEncount2* this, GlobalContext* globalCtx) {
    ...
    } else if ((this->actor.xzDistFromLink < 700.0f) && Flags_GetSwitch(globalCtx, 0x37) &&
               ((globalCtx->sceneNum == 0x4F) || (globalCtx->sceneNum == 0x1A) || (globalCtx->sceneNum == 0xE) ||
                (globalCtx->sceneNum == 0xF)) &&
               (this->unk_15C == 0)) {
        sp5E = 1;
    ...
    if (sp60 != 0) {
        ...
        if ((this->unk_158 < sp5E) && (this->unk_156 == 0)) {
            ...
            fireRock = (EnFireRock*)Actor_SpawnAttached(&globalCtx->actorCtx, &this->actor, globalCtx,
                                                        ACTOR_EN_FIRE_ROCK, sp7C, sp88, sp74, 0, 0, 0, sp62);
            if (fireRock != NULL) {
    ...
    if (objIdx >= 0) {
        ...
        for (i = 0; i < 50; i++, ptr++) {
            if (ptr->unk_10 != 0) {
                ...
                gSPMatrix(gfxCtx->polyOpa.p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_encount2.c", 0x29D),
