...
void EnKakasi2_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    param3 = (params >> 6) & 0xFF;
    this->unk_196 = params & 0x3F;
    if (param3) {};
    rotZ = this->actor.posRot.rot.z;

    if (this->unk_196 == 0x3F) {
        this->unk_196 = -1;
    }

    this->actor.unk_1F = 4;
    this->unk_19C.x = (param3 * 40.0f) + 40.0f;
    this->unk_19C.y = (rotZ * 40.0f) + 40.0f;

    // Original? (args 0)
    osSyncPrintf(VT_FGCOL(YELLOW) "☆☆☆☆☆ 元？(引数０) ☆☆☆☆ %f\n" VT_RST, param3);
    // Original? (z angle)
    osSyncPrintf(VT_FGCOL(YELLOW) "☆☆☆☆☆ 元？(Ｚアングル) ☆☆ %f\n" VT_RST, rotZ);
    // Correction coordinate X
    osSyncPrintf(VT_FGCOL(YELLOW) "☆☆☆☆☆ 補正座標Ｘ ☆☆☆☆☆ %f\n" VT_RST, this->unk_19C.x);
    // Correction coordinate Y
    osSyncPrintf(VT_FGCOL(YELLOW) "☆☆☆☆☆ 補正座標Ｙ ☆☆☆☆☆ %f\n" VT_RST, this->unk_19C.y);
    // Correction coordinate Z
    osSyncPrintf(VT_FGCOL(YELLOW) "☆☆☆☆☆ 補正座標Ｚ ☆☆☆☆☆ %f\n" VT_RST, this->unk_19C.z);
    ...
    if ((this->unk_196 >= 0) && (Flags_GetSwitch(globalCtx, this->unk_196))) {
...
void func_80A90264(EnKakasi2* this, GlobalContext* globalCtx) {
    ...
    this->unk_194++;
    if ((BREG(1) != 0) && (this->actor.xzDistFromLink < this->unk_19C.x) &&
        (fabsf(player->actor.posRot.pos.y - this->actor.posRot.pos.y) < this->unk_19C.y)) {
        this->actor.draw = func_80A90948;
        Collider_InitCylinder(globalCtx, &this->collider);
        Collider_SetCylinder(globalCtx, &this->collider, &this->actor, &sCylinderInit);
        SkelAnime_InitSV(globalCtx, &this->skelAnime, &D_060065B0, &D_06000214, 0, 0, 0);
    ...
    } else if ((this->actor.xzDistFromLink < this->unk_19C.x) &&
               (fabsf(player->actor.posRot.pos.y - this->actor.posRot.pos.y) < this->unk_19C.y) &&
...
void func_80A9062C(EnKakasi2* this, GlobalContext* globalCtx) {
    ...
    SkelAnime_ChangeAnim(&this->skelAnime, &D_06000214, 0.0f, 0.0f, (s16)frame, 2, -10.0f);
...
void EnKakasi2_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Actor_SetHeight(&this->actor, this->unk_1A8);
    ...
    if (BREG(0) != 0) {
        ...
        if (this->actor.draw == NULL) {
            if (this->unk_194 != 0) {
                ...
                DebugDisplay_AddObject(this->actor.posRot.pos.x, this->actor.posRot.pos.y, this->actor.posRot.pos.z,
                                       this->actor.posRot.rot.x, this->actor.posRot.rot.y, this->actor.posRot.rot.z,
                                       1.0f, 1.0f, 1.0f, 0, 0xFF, 0xFF, 0xFF, 4, globalCtx->state.gfxCtx);
            }
        }
    }
    if (globalCtx) {};
